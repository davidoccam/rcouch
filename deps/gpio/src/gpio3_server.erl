...
start_link() ->
gen_server:start_link({local, gpio3_server}, gpio3_server, [], []).
...
init(_Args) ->
io:format("init gpio3_server V1.0~n", []),
process_flag(trap_exit, true),
{ok,{}}.


handle_call({on_timed,Time_on}, {From, State},_) ->
wpi:pin_mode(3,output),
wpi:digital_write(3,1),
case timer:apply_after(Time_on, gpio3_server, off,[]) of
Any->ok
...
wpi:digital_write(3,0),
  {reply,ok,State};

handle_call({direction, Reset_value},{ From, State},_) ->
%io:format("Gpio3 Reset_value = ~p~n,From =  ~p~n,State ~p~n",[Reset_value, From, State]),
